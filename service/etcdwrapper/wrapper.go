...
func Set(ctx context.Context, key, value string, withLease bool) (uint64, int64, error) {
	...
	if withLease {
		...
		if err != nil {
			return 0, 0, errors.Wrapf(ctx, err, "Unable to create lease for host registration")
		}
...
func Watch(ctx context.Context, key string, idxV2 uint64, idxV3 int64) <-chan WatchResponse {
	watchChan := make(chan WatchResponse)

	go watchV2(ctx, key, idxV2, watchChan)
	go watchV3(ctx, key, idxV3, watchChan)

	return watchChan
}

func watchV2(ctx context.Context, serviceKey string, id uint64, watchChan chan WatchResponse) {
	log := logger.Get(ctx)
	...
	for {
		...
		if err != nil {
			...
			log.WithError(err).Errorf("lost watcher of '%v': '%v' (v2: %v)", serviceKey, err, client().Endpoints())
			id = 0
		...
		if err != nil {
			log.Errorf("error while getting service key '%v': '%v' (v2: %v)", serviceKey, err, client().Endpoints())
			time.Sleep(1 * time.Second)
			continue
		}
...
func watchV3(ctx context.Context, serviceKey string, rev int64, watchChan chan WatchResponse) {
	log := logger.Get(ctx)
	...
	for {
		...
		for resp := range rch {
			if resp.Canceled {
				...
				log.Errorf("lost watcher of '%v': '%v' (v2: %v)", serviceKey, resp.Err(), clientV3().Endpoints())
				time.Sleep(1 * time.Second)
			...
			for _, ev := range resp.Events {
				...
				if err != nil {
					log.WithError(err).Errorf("error while getting service key '%v': '%v' (%v)", serviceKey, err, clientV3().Endpoints())
					time.Sleep(1 * time.Second)
					continue
				}
				// We've got the modification, send it to the register agent
...
func ListValuesForService(ctx context.Context, name string) ([][]byte, error) {
	...
	if err != nil && !etcd.IsKeyNotFound(err) {
		return nil, errors.Wrapf(ctx, err, "Unable to fetch v2 services")
	}

	resv3, err := KAPIV3().Get(ctx, "/services/"+name)
	if err != nil {
		return nil, errors.Wrapf(ctx, err, "Unable to fetch v3 services")
	}
